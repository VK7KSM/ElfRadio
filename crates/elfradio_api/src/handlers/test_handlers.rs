...
use elfradio_types::{TestTtsRequest, TestSttRequest, AiError as ElfAiError};
use crate::error::ApiError;
use elfradio_core::AppState;

/// Helper to map elfradio_types::AiError to crate::error::ApiError
...
pub async fn test_stt_handler(
    ...
    let audio_bytes = BASE64_STANDARD.decode(&payload.audio_base64)
        .map_err(|e| {
    ...
    if let Some(client) = aux_client_guard.as_ref() {
        match client.speech_to_text(&audio_bytes, payload.sample_rate_hertz, &payload.language_code).await {
            Ok(transcript) => {
                ...
                Ok(Json(json!({ "status": "success", "transcript": transcript })))
            }
            Err(ai_error) => {
